...
var chat = {
  ...
  initSocket: function() {
    ...
    chat.socket = io.connect("http://128.237.216.216:8888");
    chat.listen();
    chat.initUserSocket();
...
var log = {
  ...
  displayEvents: function() {
    ...
    for (var i = 0; i < log.group.events.length; i++) {
      log.displayEvent(log.group.events[i], container);
    }
  },

  displayEvent: function(event, container) {
      var li = $("<li>");
...
<<<<<<< HEAD:static/shit.js
      ...
      nameDiv.css("font-weight", "bold");
=======
      nameDiv.html(event.name);
      nameDiv.css("padding", "10px");
      nameDiv.css("font-size", "18px");
      nameDiv.css("font-weight", "bold");
      nameDiv.css("display", "block");            
      nameDiv.attr("id", event.name);      
>>>>>>> final submission:static/client.js
      li.append(nameDiv);
      var dateDiv = $("<div>");
      dateDiv.html("Date: " + event.date);
      dateDiv.css("padding", "10px");
      dateDiv.css("font-size", "18px");
      dateDiv.css("display", "none");            
      //dateDiv.attr("class", "unclickedDiv fade");      
      li.append(dateDiv);
      var timeDiv = $("<div>");
      timeDiv.html("Start: " + event.start + ", End: " + event.end);
      timeDiv.css("padding", "10px");
      timeDiv.css("font-size", "18px");
      timeDiv.css("display", "none");      
      //timeDiv.attr("class", "unclickedDiv fade");      
      li.append(timeDiv);
      var createdDiv = $("<div>");
      createdDiv.html("Created: " + event.created);
      createdDiv.css("padding", "10px");
      createdDiv.css("font-size", "18px");
      createdDiv.css("display", "none");      
      //createdDiv.attr("class", "unclickedDiv fade");            
      li.append(createdDiv);
      li.attr("id", event.created);
      li.attr("class", "unclicked");      
      li.mousedown(function() {
        console.log("single click");
        if (li.attr("class") === "unclicked") {
          dateDiv.css("display", "block");            
          timeDiv.css("display", "block");      
          createdDiv.css("display", "block");      
          $(this).css("background-color", "#99FFCC");
          li.attr("class", "clicked");
        }
        else if (li.attr("class") === "clicked") {
